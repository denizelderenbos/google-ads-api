...
import { GoogleAdsApi, ResourceNames, enums, resources } from '../../src'
...
async function main() {
  ...
  const campaign = new resources.Campaign({
    name: 'Planet Express',
    campaign_budget: ResourceNames.campaignBudget(customer.credentials.customer_id, '123'),
  ...
  // Update the campaign
  await customer.campaigns.update([{ ...campaign, resource_name, name: 'Zapp Brannigan' }])
