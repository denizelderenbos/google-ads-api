...
export class Customer extends ServiceFactory {
  ...
  */
  ...
  /**
    ...
    @hooks onQueryStart, onQueryError, onQueryEnd
  */
  public async report<T = services.IGoogleAdsRow[]>(options: Readonly<ReportOptions>): Promise<T> {
    const { gaqlQuery, requestOptions } = buildQuery(options)
    const { response } = await this.querier<T>(gaqlQuery, requestOptions, options)
    return response
  }
  ...
  /**
    ...
    @hooks none
  */
  public async reportCount(options: Readonly<ReportOptions>): Promise<number | undefined> {
  ...
  */
  ...
  private get googleAdsFields() {
    return {
      searchGoogleAdsFields: async (request: services.SearchGoogleAdsFieldsRequest) => {
