 ...
 */
export function capitaliseFirstLetter<T extends string>(str: string): Capitalize<T> {
...
/**
 * @param customerId The Google Ads customer ID in string format (e.g., "111-222-3333")
 * @description Converts a formatted Google Ads customer ID to an integer by removing dashes.
 * @returns {number} The customer ID as an integer (e.g., 1112223333)
 * @throws {Error} If the input does not match the expected format.
 * @example
 * const customerId = formatGoogleAdsCustomerId("111-222-3333") // 1112223333
 */
export function formatGoogleAdsCustomerId(customerId: string): number {
  if (!/^\d{3}-\d{3}-\d{4}$/.test(customerId)) {
    throw new Error('Invalid customer ID format. Expected format: "111-222-3333".')
  }
  return parseInt(customerId.replace(/-/g, ''), 10)
}
